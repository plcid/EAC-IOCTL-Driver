...
namespace utils
{
	...
	PVOID get_base_address(UINT64 pid)
	{
		if (pid) return 0;
		PEPROCESS process = 0;
	...
	NTSTATUS write_process_memory(info_t* in)
	{
		if (!in->target_pid) return STATUS_UNSUCCESSFUL;
		PEPROCESS process = 0;
		PsLookupProcessByProcessId((HANDLE)in->target_pid, &process);
		if (!process) return STATUS_UNSUCCESSFUL;
		uintptr_t process_base = get_process_cr3(process);
		ObDereferenceObject(process);
		uintptr_t physical_address = translate_linear(process_base, (uintptr_t)in->target_address);
		...
		write_physical_memory((PVOID)physical_address, (PVOID)in->buffer_address, final_size, &bytes_trough);
		return STATUS_SUCCESS;
	}
	NTSTATUS read_process_memory(info_t* in)
	{
		if (!in->target_pid) return STATUS_UNSUCCESSFUL;
		PEPROCESS process = 0;
		PsLookupProcessByProcessId((HANDLE)in->target_pid, &process);
		...
		uintptr_t final_size = min(PAGE_SIZE - (physical_address & 0xFFF), in->size);
		SIZE_T bytes_trough = 0;
		read_physical_memory((PVOID)physical_address, (PVOID)in->buffer_address, final_size, &bytes_trough);
		return STATUS_SUCCESS;
	}
}
